...
[colors, MI, Mkind, TargetDir_N, TargetDir_N2, PD_N, PD_N2] = setupEnvironment(Monk, CodeDir);
...
savePath = sprintf('%s\\Figures\\Monk_%s\\', CodeDir, Monk);
...
plotSTA(sta, threshold, XTick, XTickL, (-199:0)/10, 'Membrane Potential (a.u.)', [CodeDir '\Figures\'], Monk, '20ms');

%% Make 5ms STA figure
XTick = -5:1:0;
XTickL = cellstr(string(XTick));
plotSTA(sta(:, end-49:end), threshold, XTick, XTickL, (-49:0)/10, 'Membrane Potential (a.u.)', [CodeDir '\Figures\'], Monk, '05ms');
...
save([CodeDir '\Data\' 'Monk' Monk '_FR_W_histdata.mat'],'FRW')
close all
