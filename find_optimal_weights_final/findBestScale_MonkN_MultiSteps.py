# %% import libraries
import numpy as np
import Optimal_weights_hybrid as OW
import os
...
with open(monk + 'Data/' + monk + '_weights[' + 'Aug3' + '].npy', 'rb') as f:
	weights = np.load(f)
...
#weights = OW.make_weights(weights1_multi, weights2_multi, weights3_multi, weights4_multi)
...
#         p.map(OW.get_rmse_v2, new_iterable)
...
#	    mrmse= OW.get_rmse(unit,weights,inp_indices,inp_spikes,r_all,events,duration,w_steps[ws])
